        ...
        <div className='grid gap-4 py-2'>
          <div className='grid gap-2'>
            ...
            {isSuperAdmin(currentUser) ?
              <Select
                value={role}
                onValueChange={(value) => setRole(value as StaffRow['role'])}
              >
...
export default function StaffTable({ data, onAddStaff }: StaffTableProps) {
  ...
  const { page, pageSize, pageCount, start, end, setPage, setPageSize } =
    usePagination({
      ...
      <div className='mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between'>
        <div className='relative w-full max-w-xs'>
          ...
          <input
            ...
            className='w-full rounded-md border border-slate-200 bg-white py-2 pl-9 pr-3 text-sm text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/30'
          />
        </div>
